...
// Solution method: geometric calculation
...
bool intersection (int xa, int ya, int xb, int yb,
                   int xc, int yc, int xd, int yd, double& x, double& y)
{ int d = (xa-xb)*(yc-yd) - (xc-xd)*(ya-yb);
  if (d == 0)
    return false;
  x = -((long long)(xa-xb)*(xc*yd-xd*yc) - (long long)(xc-xd)*(xa*yb-xb*ya)) / (double)(d);
  y =  ((long long)(ya-yb)*(yc*xd-yd*xc) - (long long)(yc-yd)*(ya*xb-yb*xa)) / (double)(d);
  return true;
}

// Returns the area of a polygon
template <class number>
double area (int n, number x[], number y[])
{ double A = 0;
  int i, j;
  for (i = 0; i < n; i++)
  { j = (i+1) % n;
