// reference:-https://codeforces.com/contest/1093/submission/47229079
// search_buckets provides two operations on an array:
// 1) set array[i] = x
// 2) count how many i in [start, end) satisfy array[i] < value
// Both operations take sqrt(N log N) time. Amazingly, because of the cache
// efficiency this is faster than the (log N)^2 algorithm until N = 2-5 million.
...
template <typename T> struct search_buckets {
  ...
  search_buckets(const vector<T> &initial = {}) { init(initial); }
  ...
  int prefix_less_than(int n, T value) const { return less_than(0, n, value); }
  ...
  void modify(int index, T value) {
    ...
    if (old_pos < new_pos) {
      ...
      new_pos--;
      // memmove(&buckets[old_pos], &buckets[old_pos + 1], (new_pos - old_pos) *
      // sizeof(T));
    } else {
      copy_backward(buckets.begin() + new_pos, buckets.begin() + old_pos,
                    buckets.begin() + old_pos + 1);
      // memmove(&buckets[new_pos + 1], &buckets[new_pos], (old_pos - new_pos) *
      // sizeof(T));
    }
